...
- (void)updatePageControl {
    ...
    if (self.rotatorType == PXPotatorWhiteDot){
        [self.pageControl setThumbImage:[self createDot:[UIColor colorWithRed:220.0f/255.0f green:154.0f/255.0f blue:39.0f/255.0f alpha:1.0]]];
    ...
    if (self.rotatorType == PXRotatorNumber) {
        for (int i = 0; i < self.carousel.numberOfItems; i++) {
            ...
            [self.pageControl setThumbImage:[self createDot:[UIColor colorWithRed:220.0f/255.0f green:154.0f/255.0f blue:39.0f/255.0f alpha:1.0]] forIndex:i];
            //UIImage *selected = [self createText:num withColor:[UIColor whiteColor] backgroundColor:[UIColor redColor]];
            [self.pageControl setSelectedThumbImage:[self createDot:[UIColor whiteColor]] forIndex:i];
        }
    }
}
...
- (UIImage *)createText:(NSString *)text withColor:(UIColor *)textColor backgroundColor:(UIColor*)bgColor {
    ...
    [text drawInRect:CGRectMake(xOffset, yOffset, 5, font.pointSize) withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:textColor}];
